#!/usr/bin/env bash
set -euo pipefail

# Run Kinship path-fidelity baselines from the repo root.
# Usage:
#   bash scripts/baselines/run_kinship.sh
#   bash scripts/baselines/run_kinship.sh 0 42 100

seeds=("$@")
if [[ $# -eq 0 ]]; then
  seeds=(0 42 100)
fi

repo_root="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
cd "$repo_root"

output_dir="output/kinship/baselines"
mkdir -p "$output_dir"

common_args=(
  --data-input-dir ./datasets/nlq/kinship/
  --question-path ./datasets/nlq/kinship/kinship_qa_nhop.csv
  --cached-qa-metadata-path ./.cache/itl/kinship_qa_nhop.json
  --test-only
  --use-self-loops
  --use-full-graph
  --num-rollout-steps 3
  --max-num-actions 100
)

for seed in "${seeds[@]}"; do
  conda run -n minerva_tf2 python -m code.baselines.random_walk_stats \
    "${common_args[@]}" \
    --num-walks 100 \
    --seed "$seed" \
    --output "$output_dir/random_walk_stats_kinship_seed${seed}.json"

  conda run -n minerva_tf2 python -m code.baselines.shortcut_oracle_stats \
    "${common_args[@]}" \
    --seed "$seed" \
    --output "$output_dir/shortcut_oracle_stats_kinship_seed${seed}.json"
done
